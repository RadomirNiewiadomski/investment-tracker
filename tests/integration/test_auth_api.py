...
async def test_register_duplicate_email(client: AsyncClient):
    ...
    assert json_response["error"]["message"] == "Email already registered"
...
async def test_login_invalid_credentials(client: AsyncClient):
    ...
    assert json_response["error"]["message"] == "Incorrect email or password"
...
async def test_read_users_me_unauthorized(client: AsyncClient):
    ...
    assert response.status_code == 401
    json_response = response.json()
    assert json_response["error"]["message"] == "Not authenticated"
